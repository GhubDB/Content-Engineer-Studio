        ...
        for ix, item in enumerate(new_columns):
            ...
            if dup[ix]:
                for level in range(len(new_columns[ix])):
                    new_columns[ix][level] = (
                        new_columns[ix][level] + f"_{suffix[tuple(item_orig)]}"
                    )
    ...
    if issubclass(type(df.columns), pd.core.indexes.multi.MultiIndex):
        ...
        for level in levels:
            if any([type(val) != str for val in level]):
                logger.warning(
                    f"In {name}, converted MultiIndex level values to string in: {str(level)}"
                )
                df.columns = df.columns.set_levels(
                    [[str(val) for val in level] for level in levels]
                )
                converted_names.append(str(level))
        if converted_names:
            logger.warning(
                f"In {name}, converted MultiIndex level names to string: {', '.join(converted_names)}"
            )
    else:
        ...
        if converted_names:
            logger.warning(
                f"In {name}, converted column names to string: {', '.join(converted_names)}"
            )

    # Check for duplicate columns
    if any(df.columns.duplicated()):
        logger.warning(
            f"In {name}, renamed duplicate columns: {list(set(df.columns[df.columns.duplicated()]))}"
        )
...
def resize_widget(widget, x, y):
    ...
    widget.resize(
        QtCore.QSize(
            int(x * QtWidgets.QDesktopWidget().screenGeometry().width()),
...
def get_figure_type(fig):
    ...
    try:
        ...
        if issubclass(type(fig), matplotlib.axes.Axes) or issubclass(
...
def parse_cell(text, column_dtype):
    ...
    try:
        ...
        value = pd.read_csv(StringIO(text), dtype=column_dtype, header=None).values[0][
            0
        ]
        return value
    except ValueError:
...
def summarize_json(data, terse=True):
    ...
    def list_keys(data, parent_key="ROOT"):
        ...
        if isinstance(data, list):
            ...
            for item in data:
                keys += list_keys(
                    item, f'{".".join(parent_subkeys[:-1])}.[{parent_subkeys[-1]}]'
                )
    ...
    for key, count in keycount.items():
        if terse:
            ...
            for redundant_subkey in sorted(
                list(redundant_subkeys), key=len, reverse=True
            ):
